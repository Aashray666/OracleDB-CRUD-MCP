...
# import re
# import json # Import json to parse arguments string
...
#     model_name="llama-3.3-70b-versatile" # Using a Groq model as per your previous successful traces
# )
...
#     Prints a human-readable summary of the LLM's response.
#     """
...
#             "- **STRICT ADHERENCE:** When a user requests a database operation, you MUST strictly adhere to the exact names, values, and parameters provided in the user's prompt. Do NOT invent, change, or omit any details unless explicitly instructed.\n"
#             "- **AVOID REDUNDANCY:** Do NOT make multiple identical tool calls for the same request. Optimize your tool usage to be efficient.\n"
...
#     # --- Start of Human-Readable Output Formatting ---
#     print("--- LLM Response (Human-Readable) ---", flush=True)
#     if response.tool_calls:
#         print("LLM decided to call tool(s):", flush=True)
#         for tool_call in response.tool_calls:
#             # CORRECTED: Access attributes using dictionary keys as 'tool_call' is a dict
#             # The structure is typically {'name': 'tool_name', 'args': {...}, 'id': '...'}
#             tool_name = tool_call['name']
#             tool_args = tool_call['args'] # This is already a dictionary
#             tool_id = tool_call['id']
            
#             print(f"  Tool Name: {tool_name}", flush=True)
#             print(f"  Arguments: {tool_args}", flush=True)
#             print(f"  Tool Call ID: {tool_id}", flush=True)
#     elif response.content:
#         print("LLM generated a direct message:", flush=True)
#         print(f"  Content: {response.content}", flush=True)
#     else:
#         print("LLM response was empty or unexpected.", flush=True)
#     print("-------------------------------------", flush=True)
#     # --- End of Human-Readable Output Formatting ---
...
#         {"messages": [HumanMessage(content="Create a user named 'Alice' with email 'alice@example.com' and age 30.")]},
...
model = ChatGoogleGenerativeAI(
    ...
    temperature=0, # Keep temperature at 0 for deterministic tool calls
    google_api_key=os.getenv("GEMINI_API_KEY") # If you prefer to load from .env
)
...
def call_model(state: MessagesState):
    ...
    messages_with_system_prompt = [
        SystemMessage(content=(
            ...
            "Your available tools are: 'create_record', 'read_records', 'update_record', and 'delete_record'.\n\n"
            "**CRITICAL GUIDELINES FOR TOOL USAGE:**\n"
            "- **TOOL INVOCATION ONLY:** If you need to perform a database operation (Create, Read, Update, Delete), "
            "  you MUST respond with a valid tool call object (e.g., `tool_calls=[ToolCall(...)]`). "
            "  DO NOT output tool call syntax (like '<function=...>') directly in your text content. "
            "  Your text content should be EMPTY if you are making a tool call.\n"
            "- **STRICT ADHERENCE:** When a user requests a database operation, you MUST strictly adhere to the exact names, "
            "  values, and parameters provided in the user's prompt. Do NOT invent, change, or omit any details unless explicitly instructed.\n"
            "- **AVOID REDUNDANCY:** Do NOT make multiple identical tool calls for the same request. Optimize your tool usage to be efficient.\n"
            "- **Table Names:** Refer to tables as 'USERS' or 'PRODUCTS' (uppercase is best practice for Oracle).\n"
            "- **'create_record' tool:**\n"
            "  - Use for adding new entries. Provide 'table_name' (e.g., 'USERS') and 'data' (a dictionary).\n"
            ...
            "  - Use for removing records. Provide 'table_name' and 'record_id'.\n\n"
            "**RESPONSE GUIDELINES:**\n"
            "- **Summarize Tool Output:** After a tool has been executed, you will receive its output as a `ToolMessage`. Your next response MUST be a clear, concise summary of that output for the user in natural language.\n"
            "- **Final Answer:** If you have fully answered the user's request and no more tool calls are needed, provide your final answer in natural language and stop generating tool calls.\n"
            "- **Empty Results:** If a tool operation returns an empty list (e.g., from 'read_records'), state that no records were found.\n"
            "- **Error Reporting:** If a tool returns an error, report the error message to the user and suggest how they might rephrase their request or what might be wrong (e.g., 'table not found', 'invalid ID')."
        ))
    ] + state["messages"] # Prepend the system message to the existing messages
    ...
    if response.tool_calls:
        ...
        print("LLM response was empty or unexpected (e.g., tool call with no content).", flush=True)
    print("-------------------------------------", flush=True)
    # --- End of Human-Readable Output Formatting ---
